...
class JSON {
	 ...
	 * @param outputType class name to parse to, defaults to Object
	 */
	static parse(text, keepbooltype := false, as_map := true, outputType := Object) {
		keepbooltype ? (_true := this.true, _false := this.false, _null := this.null) : (_true := true, _false := false, _null := "")
		as_map ? (map_set := (maptype := Map).Prototype.Set) : (map_set := (obj, key, val) => obj.%key% := val, maptype := outputType)
